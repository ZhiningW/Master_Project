...
| $\mathds{R}^{p}$ | the space of all p-dimensional real column vectors like $[a_1,a_2,\dots,a_p]^T$ |
| $\mathds{R}^{p\times q}$ | the space of all real matrices with size $p\times q$ - $\mathds{E}[\cdot]$ as the expectation, or mean of a random variable |
| $\mathds{V}[\cdot]$ | the variance, or covariance of a random variable |
